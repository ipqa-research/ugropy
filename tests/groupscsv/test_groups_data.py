...
from ugropy import dortmund, psrk, unifac
...
def test_dortmund_data():
    ...
    for group in df.index:
        assert df.loc[group, "R"] == dortmund.subgroups_info.loc[group, "R"]
        assert df.loc[group, "Q"] == dortmund.subgroups_info.loc[group, "Q"]
        assert (
            df.loc[group, "No."]
            == dortmund.subgroups_info.loc[group, "subgroup_number"]
        )

        assert (
            df.loc[group, "Main Group No."]
            == dortmund.subgroups_info.loc[group, "main_group"]
        )
