...
def test_filter_mostly_polyatomic():
    sol = ug.unifac.get_groups(ester, "smiles", search_multiple_solutions=True)
    fil = ug.unifac.filter_mostly_polyatomic(sol)

    assert fil[0].subgroups == {"CH3": 1, "ACH": 5, "ACCH2": 1, "COO": 1}
...
def test_filter_bigger_polyatomics():
    sol1 = ug.unifac.get_groups(
        ester, "smiles", search_multiple_solutions=True
    )
    fil1 = ug.unifac.filter_bigger_polyatomics(sol1, "Q")
    assert fil1[0].subgroups == {"CH3": 1, "ACH": 5, "ACCH2": 1, "COO": 1}

    fil2 = ug.unifac.filter_bigger_polyatomics(sol1, "R")
    assert fil2[0].subgroups == {"CH3": 1, "ACH": 5, "ACCH2": 1, "COO": 1}

    with pytest.raises(ValueError):
        ug.unifac.filter_bigger_polyatomics(sol1, "invalid")
